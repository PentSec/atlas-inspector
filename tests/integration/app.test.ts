...
beforeAll(async () => {
    ...
    config = loadConfig({
        ...
        LOG_PRETTY: "0",
        // No network in tests. Nothing in the read path downloads any more, and
        // the listfile is injected explicitly per test; the release checker is
        // only started by the process entry point, never by buildApp.
    });
...
describe("/api/v1", () => {
    ...
    it("never downloads on a cold install: search and health are side-effect free", async () => {
        // The single most important assertion in this file. A cold install must
        // report "absent" and leave the disk alone; the 146 MB download may only
        // ever start from POST /api/v1/listfile/fetch, which the MCP layer calls
        // after the user approved it.
        const csv = path.join(tmp, "cold.csv");
        const fetcher = vi.fn(async () => ({ bytes: 1 }));
        app = await makeApp({ listfile: new ListfileIndex({ csvPath: csv, fetch: fetcher }) });

        await app.inject({ method: "GET", url: "/api/v1/search?q=foo&wait=25" });
        await app.inject({ method: "GET", url: "/api/v1/health" });
        await app.inject({ method: "GET", url: "/api/v1/search?q=foo" });

        expect(fetcher).not.toHaveBeenCalled();
    });

    it("POST /listfile/fetch is the only path that downloads", async () => {
        const csv = path.join(tmp, "consent.csv");
        const fetcher = vi.fn(async () => {
            await writeFile(csv, "134400;interface/icons/inv_misc_questionmark.blp\n");
            return { bytes: 64, release: "202609242243" };
        });
        app = await makeApp({ listfile: new ListfileIndex({ csvPath: csv, fetch: fetcher }) });

        const res = await app.inject({
            method: "POST",
            url: "/api/v1/listfile/fetch",
            payload: { wait: 5 },
        });
        expect(res.statusCode).toBe(200);
        ...
        const cap = res.json();
        expect(cap).toMatchObject({ status: "ready", entries: 1, release: "202609242243" });

        // And the index is genuinely usable afterwards.
        const search = await app.inject({ method: "GET", url: "/api/v1/search?q=questionmark" });
        expect(search.json()).toMatchObject({ status: "ready" });
    });

    it("POST /listfile/fetch with no body is a bounded, non-hanging trigger", async () => {
        const csv = path.join(tmp, "nobody.csv");
        let release!: () => void;
        ...
        // wait defaults to 0, so the route acknowledges immediately instead of
        // holding an HTTP connection open for a multi-minute download.
        const res = await app.inject({
            method: "POST",
            url: "/api/v1/listfile/fetch",
            payload: {},
        ...
        expect(res.statusCode).toBe(200);
        expect(res.json()).toMatchObject({ status: "indexing" });
        expect(fetcher).toHaveBeenCalledTimes(1);

        release();
    });

    it("POST /listfile/fetch rejects an out-of-range wait", async () => {
        const csv = path.join(tmp, "badwait.csv");
        const fetcher = vi.fn(async () => ({ bytes: 1 }));
        app = await makeApp({ listfile: new ListfileIndex({ csvPath: csv, fetch: fetcher }) });
    ...
    it("POST /listfile/fetch fails closed when the build cannot download", async () => {
        const csv = path.join(tmp, "nofetcher.csv");
        app = await makeApp({ listfile: new ListfileIndex({ csvPath: csv }) });
    ...
    it("search reports the index state instead of a 503 (ADR-014)", async () => {
        app = await makeApp();
        const res = await app.inject({ method: "GET", url: "/api/v1/search?q=foo" });

        // 200 + explicit status: "not built yet" is a state, not a transport
        // failure, and the old 503 pushed recovery onto a human who had to run npm.
        expect(res.statusCode).toBe(200);
        const body = res.json();
        expect(body.status).toBe("absent");
        expect(body.hits).toEqual([]);
        expect(body.detail).toMatch(/fetch:listfile/);
    });

    it("search answers hits once an index is present", async () => {
        const csv = path.join(tmp, "listfile-ready.csv");
        await writeFile(csv, "134400;interface/icons/inv_misc_questionmark.blp\n");
        app = await makeApp({ listfile: new ListfileIndex({ csvPath: csv }) });

        const res = await app.inject({ method: "GET", url: "/api/v1/search?q=questionmark" });
        expect(res.statusCode).toBe(200);
        expect(res.json()).toMatchObject({
            status: "ready",
            hits: [{ filedata: 134400, name: "interface/icons/inv_misc_questionmark.blp" }],
        });
    });

    it("search rejects an out-of-range wait and never blocks past the cap", async () => {
        app = await makeApp();

        // Out of range is a client error, not a silent clamp: quietly ignoring
        // wait=99999 would hide the fact that the caller never got the wait it asked for.
        const tooLong = await app.inject({ method: "GET", url: "/api/v1/search?q=foo&wait=99999" });
        expect(tooLong.statusCode).toBe(400);

        // In range: nothing is authorized, so it returns immediately regardless
        // of the cap. This is the whole point of the consent gate.
        const started = Date.now();
        const res = await app.inject({ method: "GET", url: "/api/v1/search?q=foo&wait=25" });
        expect(res.statusCode).toBe(200);
        expect(res.json()).toMatchObject({ status: "absent" });
        expect(Date.now() - started).toBeLessThan(2_000);
    });

    it("health publishes search readiness so status:ok is never read as 'all tools work'", async () => {
        app = await makeApp();
        const res = await app.inject({ method: "GET", url: "/api/v1/health" });
        expect(res.statusCode).toBe(200);

        const body = res.json();
        // Liveness is unchanged and still "ok" — a cold index does not kill the process.
        expect(body).toMatchObject({ status: "ok", service: "atlas-inspector" });
        // Readiness is a separate, honest signal.
        expect(body.capabilities.search).toMatchObject({ status: "absent", entries: 0 });
        expect(body.capabilities.search.detail).toBeTruthy();
    });
