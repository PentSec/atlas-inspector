...
describe("parseBlpHeader (real BLP2 fixtures)", () => {
  it("reads the 1024x1024 DXT5-with-alpha atlas", () => {
    ...
    expect(h).toMatchObject({ compressionId: 2, alphaSize: 8, preferredFormat: 7, mipmaps: 1, width: 1024, height: 1024 });
  ...
  it("reads the 64x64 DXT1 icon with 7 mipmaps", () => {
    ...
    expect(h).toMatchObject({ compressionId: 2, preferredFormat: 0, mipmaps: 7, width: 64, height: 64 });
  ...
  it("reads the 8x8 plain texture", () => {
    ...
    expect(h).toMatchObject({ compressionId: 2, preferredFormat: 0, mipmaps: 4, width: 8, height: 8 });
    expect(compressionLabel(h)).toBe("dxt1");
    expect(blpAlphaDepth(h)).toBe(1);
  });
