...
export function renderTexture(store: AppStore, j: TextureResult): void {
    const versions =
        j.versions && j.versions.length ? j.versions.slice(0, 5).join(", ") : j.version || "?";
    setMetaHTML(
    ...
    td.colSpan = 3;
    td.innerHTML = `<span style="color:var(--dim)">Single-file texture, no UiTextureAtlasMember regions.</span> `;
...
export function updateExportButtons(store: AppStore): void {
    ...
    const f = s.file;
    dl.disabled = !f;
    dl.title = f ? `Download ${f.fdid}.blp${f.version ? ` (build ${f.version})` : ""}` : "";
}
