...
export const legacyRoutes: FastifyPluginAsync<LegacyRouteOptions> = async (fastify, opts) => {
  ...
  fastify.get("/atlas", async (req, reply) => {
    ...
    const result = await repo.resolve(
      Number(raw),
      build && build !== "default" ? build : null,
    );
    if (!result) {
      return reply
        .code(404)
      ...
      try {
        ...
        if (err instanceof UnprocessableError) {
          return reply.code(422).send({
            kind: "render",
            error:
              "BLP format not supported for preview — drop a PNG of the sheet instead",
          });
        }
        throw err;
      }
  ...
  fastify.post("/decode", async (req, reply) => {
    ...
    try {
      ...
      return reply.send({ kind: "ok", w: result.width, h: result.height, png: result.pngUrl });
    } catch (err) {
