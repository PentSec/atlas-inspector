...
import { blpAlphaDepth, compressionLabel, parseBlpHeader, pngSize } from "../adapters/blpDecoder.js";
  ...
  async decode(input: Buffer): Promise<BlpDecodeResult> {
    ...
    if (input.length > this.opts.maxBytes) {
      throw new PayloadTooLargeError(
        `BLP exceeds the ${this.opts.maxBytes}-byte limit`,
      );
    }
    ...
    try {
      ...
      header = { compressionId: -1, alphaSize: 0, preferredFormat: 0, mipmaps: 0, width: 0, height: 0 };
    }
